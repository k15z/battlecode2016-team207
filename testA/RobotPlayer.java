...
/**
 * A for CIA. Builds checker-board of turrets to hide behind and sends scouts 
...
public class RobotPlayer {
    ...
    static void archon() {
	        	...
	        	if (signal == null) {
	            	...
	            	while (src.distanceSquaredTo(dest) > 9 && attempts++ < MAX_ATTEMPTS) {
	            		...
	            		try {
	            			...
	            			//build soldiers in the beginning
			    	    	if (numSoldiers < 2 && robot.canBuild(dir, RobotType.SOLDIER)){
    	...
    	{ /* Loop. */
    		int TURRET_SCOUT = 5;            //this values is 5!
    		...
    		while (true) {
    			...
    			try {
    				...
    				if (random.nextDouble() < 1.0/AVE_NUM_ARCHONS)
	    				if (random.nextDouble() > 1.0/TURRET_SCOUT && random.nextDouble() < 1.0*(TURRET_SCOUT-1)/TURRET_SCOUT) {
		    	    	    	while (!robot.isCoreReady() || !robot.hasBuildRequirements(RobotType.TURRET)) {
		    	    	    		Clock.yield();
		    	    				if (robot.getHealth() < ESCAPE_HEALTH)
		    	    					archon_escape();
	    				...
	    				}else if(random.nextDouble() > 1.0*(TURRET_SCOUT-1)/TURRET_SCOUT){
    ...
    static void soldier() {
		...
		while (true) {
	    	try {
	    		if (robot.isCoreReady() && robot.getRoundNum() < 400) {
                    ...
                    for (int i = 0; i < robots.length; i++)
                        if(robots[i].type == RobotType.ARCHON){
                        	Direction dir = robot.getLocation().directionTo(robots[i].location);
                        	walk(dir);
                        }
	    		}
